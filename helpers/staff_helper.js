...
module.exports = {
    doLogin: (userData) => {
      ...
      updateStatus: (status,id) => {
        return new Promise(async (resolve, reject) => {
          let result = await db.get().collection('forms').updateOne({_id:objectId(id)},{$set:{status:status}})
          resolve(result)
        })
      },
    ...
    getApplications: () => {
    return new Promise(async (resolve, reject) => {
      let result = await db.get().collection('forms').find().toArray()
      resolve(result)
    })
  },
  getApplication: (appli_id) => {
    return new Promise(async (resolve, reject) => {
      let result = await db.get().collection('forms').findOne({ _id: objectId(appli_id) })
      // .then((response)=>{
        resolve(result)
      // })
    })
  },
  getApplicationStatusFilter: (status) => {
    console.log(status);
    return new Promise(async (resolve, reject) => {
      let result = await db.get().collection('forms').find({ status: status }).toArray()
      // .then((response)=>{
        resolve(result)
        // console.log(result);
      // })
